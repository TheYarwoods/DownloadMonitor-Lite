...
void ConfigManager::UpdateShowFirstRunMsg(bool newBool)
{
    shownFirstRunMsg = newBool;
    WriteData();
}
...
void ConfigManager::WriteData()
{
    ...
    char fg_Buf[200];
    sprintf_s(fg_Buf, "%s=%i,%i,%i", FOREGROUND_COLOUR, fg_r, fg_g, fg_b);

    char ch_Buf[200];
    sprintf_s(ch_Buf, "%s=%i,%i,%i", CHILD_COLOUR, ch_r, ch_g, ch_b);
    ...
    char fr_Buf[200];
    sprintf_s(fr_Buf, "%s=%i", FIRST_RUN_MSG, shownFirstRunMsg ? 1 : 0);
...
void ConfigManager::ReadData()
{
    ...
    while(getline(configFile, output))
    {
        ...
        else if (!strncmp(output.c_str(), FIRST_RUN_MSG, 13))
        {
            char* dataStart = strchr((char*)output.c_str(), '=');
            int val = ProcessInt(dataStart);
            if (val < 0 && val > 1)
            ...
            shownFirstRunMsg = (val == 1) ? true : false;
            readFirstRun = true;
        }
    }
    ...
    if(!readFgCol || !readChCol || !readLastPos || !readOpacity || !readFirstRun || invalidCfg)
    {
        if (retriedOnce)
        {
...
void ConfigManager::InitDefaults(bool resetMsgVar)
{
    char pathBuf[MAX_PATH];
    GetFullConfigPath(pathBuf);
    ...
    std::string tmpCfg(defaultConfig);

    tmpCfg.append(resetMsgVar ? "FIRST_RUN_MSG=0\n" : "FIRST_RUN_MSG=1\n");

    newConfigFile << tmpCfg.c_str();
